...
### There is no Clinical Lead — permanently, by design

**Superseded the earlier "no Clinical Lead is assigned" wording on 27 Aug 2026.
The difference matters:** it is not that the post is vacant, it is that the
role does not exist and is not coming.

**Karabo and Nicola do not share notes or case files with each other** unless
one deliberately refers a case to the other.

So `is_clinical_lead()` and `counsellors.is_clinical_lead` are **not built** —
not built-and-defaulted-to-false. **A permission nobody uses is a bug waiting
to be flipped without anyone re-deriving why it existed.** A dormant column
reads, to whoever finds it in two years, like a half-finished feature rather
than a boundary someone decided against on purpose. The absence has to be
legible in the schema itself.

Consequences, all load-bearing for M3:

- The psychosocial `bookings` branch is
  `(service_line = 'psychosocial' and counsellor_id = current_counsellor_id())
  or is_psychosocial_admin()`. **No clinical-lead disjunct.**
- `counselling_notes` is **author-only, no exception, ever.** No admin policy,
  no lead policy, no break-glass.
- **There is no read path by which one counsellor sees another's notes or
  bookings.** None.

**`counselling_referrals` is the only sanctioned path between counsellors**,
and it works by the referring counsellor **authoring something new** — never by
granting sight of notes they already wrote. Those stay theirs, permanently,
referral or not. A referral that granted access to the original notes would
make the author-only policy conditional on a row in another table, and the
boundary would then be only as strong as whoever can write that row.

Risk flags are unaffected: they were always routed to Lone, never to a clinical
lead.
