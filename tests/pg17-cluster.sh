#!/usr/bin/env bash
# Key Wellness — a throwaway PostgreSQL 17 cluster for the database suites.
#
# Production is 17.6 and the suites must run on it. Distro packages are the
# better answer where they exist; where they do not (a container with only 16,
# or one that cannot reach apt.postgresql.org) this fetches the official 17.6
# server binaries from npm — registry.npmjs.org is reachable almost everywhere
# a build runs — starts a cluster, runs the suite it is given, and stops it.
#
# Extracted from tests/run-advance-db-pg17.sh on 3 Sep 2026 so the Debt Rehab
# Plan suite uses the same cluster bootstrap rather than a second copy of it.
#
# Linux x64 only. psql comes from PATH; a 16 client against a 17 server is fine.
# Usage: tests/pg17-cluster.sh <suite-script> [port]     (default port 5434)
set -e
SUITE="${1:?usage: pg17-cluster.sh <suite-script> [port]}"
PORT="${2:-5434}"
...
SERVER_VER=$(psql -h "$SOCK" -p "$PORT" -U postgres -tAc 'show server_version_num')
echo "→ $(psql -h "$SOCK" -p "$PORT" -U postgres -tAc 'select version()' | cut -c1-40)"
[ "$SERVER_VER" -ge 170000 ] || { echo "server is below PostgreSQL 17 ($SERVER_VER)"; exit 1; }

PGUSER=postgres "$SUITE" "$SOCK" "$PORT"
