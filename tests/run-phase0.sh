...
_kwout=$($PSQL -d kwtest -f "$ROOT/migrations/rollback-org-account-phase1-indicators.sql" 2>&1) || { echo "$_kwout" | grep -vE "NOTICE:|^$"; echo "  rollback          FAILED"; exit 1; }
_kwout=$($PSQL -d kwtest -f "$ROOT/migrations/rollback-org-account-phase0a-picker.sql" 2>&1) || { echo "$_kwout" | grep -vE "NOTICE:|^$"; echo "  rollback          FAILED"; exit 1; }
_kwout=$($PSQL -d kwtest -f "$ROOT/migrations/rollback-org-account-phase0.sql" 2>&1) || { echo "$_kwout" | grep -vE "NOTICE:|^$"; echo "  rollback          FAILED"; exit 1; }
echo "  rollback          ok"
_kwout=$($PSQL -d kwtest -f "$ROOT/migrations/rollback-org-account-phase1-indicators.sql" 2>&1) || { echo "$_kwout" | grep -vE "NOTICE:|^$"; echo "  rollback          FAILED"; exit 1; }
_kwout=$($PSQL -d kwtest -f "$ROOT/migrations/rollback-org-account-phase0a-picker.sql" 2>&1) || { echo "$_kwout" | grep -vE "NOTICE:|^$"; echo "  rollback          FAILED"; exit 1; }
_kwout=$($PSQL -d kwtest -f "$ROOT/migrations/rollback-org-account-phase0.sql" 2>&1) || { echo "$_kwout" | grep -vE "NOTICE:|^$"; echo "  rollback          FAILED"; exit 1; }
