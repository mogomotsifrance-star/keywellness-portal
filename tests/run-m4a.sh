#!/bin/bash
# Key Wellness — M4a database tests against a local PostgreSQL 17.
#
...
# stand-in.
#
#   Linux/macOS :  tests/run-m4a.sh
#   Windows      :  PGUSER=postgres tests/run-m4a.sh 127.0.0.1 5433
set -e
HOST="${1:-/tmp/pgrun}"
PORT="${2:-5433}"
...
$PSQL -d postgres -c "drop database if exists kwm4a" >/dev/null
$PSQL -d postgres -c "create database kwm4a"          >/dev/null

$PSQL -d kwm4a -f "$HERE/m1-fixture.sql"       >/dev/null
$PSQL -d kwm4a -f "$HERE/m4-fixture-extra.sql" >/dev/null
echo "  fixture           ok"

$PSQL -d kwm4a -f "$ROOT/supabase_org_report_data_v4.sql" >/dev/null
...
$PSQL -d kwm4a -f "$HERE/m4-baseline.sql" >/dev/null
echo "  baseline captured ok ($($PSQL -d kwm4a -t -A -c 'select count(*) from m4_baseline') org/period payloads)"
...
$PSQL -d kwm4a -f "$HERE/m4a-tests.sql" 2>&1 \
  | grep -E "PASS|FAIL|passed|REGRESSION|STATE|ERROR|FATAL" \
  | sed "s/^psql:[^ ]* //; s/^NOTICE:  //"

$PSQL -d kwm4a -c "drop table if exists m4_baseline; drop table if exists m4_counts" >/dev/null
...
# are not.
run_rollback() {
  if ! $PSQL -d kwm4a -f "$ROOT/migrations/rollback-m4a-ownership-and-roles.sql"        2>&1 | grep -vE "NOTICE:|^$"; then :; fi
  return "${PIPESTATUS[0]}"
}
...
# is_ops_admin must be back, or M4 is broken after a rollback.
BACK=$($PSQL -d kwm4a -t -A -c "
...
if [ "$BACK" = "ok" ]; then
  echo "  is_ops_admin back ok (M4 still works after a rollback)"
else
  echo "  is_ops_admin back FAILED — M4's policies now reference nothing"; exit 1
fi

# M4a's rollback must not touch M4's tables.
ROWS=$($PSQL -d kwm4a -t -A -c "
