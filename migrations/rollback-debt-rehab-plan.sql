...
-- ============================================================
-- Removes the Debt Rehab Plan table and its five RPCs. Nothing else was
-- touched by the forward migration, so nothing else is undone.
--
-- WHAT IS LOST: every generated Debt Rehab Plan (drafts and finals) and their
-- input/computed audit snapshots. The system notes written into advisor_notes
-- ("Debt Rehab Plan v1 generated…") are left in place deliberately — they are
-- part of the client's timeline and record that a generation happened, even
-- if the plan itself is gone. Delete them by hand only if that is what you
-- want:
...
-- and remove the third Report-tab view from advisor.html (git revert).
--
-- NOTE: this does NOT undo the shared-module extraction in
-- supabase/functions/_shared/kw-finance.ts. That is a refactor of the
-- Advance Recommendation, not part of this feature, and the AR depends on it.
-- If it must also go, revert that commit and redeploy advance-recommendation.
-- ============================================================
