...
## 10. Risk escalation - settled, and nothing is built for it

**Decided 28 Aug 2026 and permanent, for confidentiality reasons.**

A counsellor escalates by **referring the client to external parties**
according to the level of risk. That happens **outside the platform**, the way
out-of-band contact always has.

There is **no internal flag, no notification and no admin action** tied to a
risk event. The content-free, nameless action for Lone that earlier versions of
this section proposed is **superseded and is not being built** - not deferred,
not owed, not pending.

Two things that are easy to reintroduce by accident:

- **`actions` must not be shaped to leave room for a risk-flag row type.**
  None is coming.
- **"Nobody is clinical lead" is no longer the reason.** Escalation would go
  outside the platform even if somebody held that role. The absence of a
  clinical lead and the absence of a risk flag are two separate permanent
  decisions that happen to point the same way.

Recorded in `claude/ops-data-model-and-impact.md` 2.3a and 5 item 2, and in
`claude/ops-migration-governance.md`'s M3b section.
