...
-- Batch 2: extend org_report_data() in place (signature unchanged)
-- Run this AFTER supabase_program_activities.sql (Batch 1 of this
-- workstream) in the Supabase SQL Editor. Safe to re-run (CREATE OR
-- REPLACE). Requires the previous workstream's SQL already live
-- (org_reports, org_report_data, publish_org_report, bookings attendance
-- columns — see BUILD-NOTES.md Batch 0 prerequisite check).
--
-- Design decisions + rollback recorded in BUILD-NOTES.md ("Batch 2 —
...
-- A percentage computed from a numerator < 3 is nearly as identifying as
-- showing the raw count (e.g. "100%" from "1 of 1"). Mirrors _suppress_count's
-- shape so callers can treat every cell in the report the same way.
...
begin
  ...
  -- Additive alongside assessment_categories (left untouched above) — one
  -- canonical dimension per tool file, no tool double-counted. See
  -- BUILD-NOTES.md for the full mapping and the two excluded tools.
  with cohort as (
    select p.id
    from profiles p
    join auth.users u on u.id = p.id
...
-- 1. Grep this file case-insensitively for the banned score-direction term
--    from points_catalog (see BUILD-NOTES.md Batch 0): zero matches.
...
--    none anywhere in the tree, including previous_period, program_activities,
--    demographics_cross.

-- 3. Complementary suppression edge case: find or construct (via a test
--    org) a period where exactly one age-band/tier cell is suppressed
--    (count 1 or 2) while the rest of that row is ≥3 — confirm the row's
--    row_total is ALSO {"value": null, "suppressed": true}, not the raw
--    sum. Then construct a row with TWO suppressed cells and confirm its
--    row_total DOES show a real number (more than one unknown = safe).

-- 4. Previous consumers still render: open an existing published report
--    (or the admin builder preview) against a real org — the current
--    fields (engagement_funnel, sessions, assessment_categories,
--    demographics, learning) must be byte-identical in shape to before
--    this batch.

-- 5. Rollback: re-apply supabase_org_report_data.sql (drops back to the
--    pre-Batch-2 function bodies via CREATE OR REPLACE).
-- ─────────────────────────────────────────────────────────────
