...
--
-- (f2) CANCELLING KEEPS THE CONFIRMATION. If Lone confirmed the invoice and
--     the handover is cancelled afterwards, an invoice very likely exists in
--     Sage and needs a credit note. Clearing invoice_confirmed_by/_at would
--     destroy the only trace of it at exactly the moment she needs to chase
--     it. WE RECORD WHAT HAPPENED, NOT WHAT IS CURRENTLY TRUE. The state reads
--     'cancelled'; the confirmation stays beside it, with the reason.
--
...
begin
  ...
  -- Who confirmed and when move together, always.
  if not exists (select 1 from pg_constraint where conname='billing_handovers_confirmed_pair') then
    alter table billing_handovers add constraint billing_handovers_confirmed_pair
      check ((invoice_confirmed_at is null) = (invoice_confirmed_by is null));
  end if;

  -- 'invoiced' REQUIRES a confirmation. 'to_prepare' and 'handed_over' FORBID
  -- one, because nobody has confirmed anything yet.
  --
  -- 'cancelled' ALLOWS either, and that is the point: if Lone confirmed the
  -- invoice and the handover was cancelled afterwards, an invoice very likely
  -- exists in Sage and needs a credit note. Clearing the confirmation would
  -- destroy the only trace of it at exactly the moment she needs to chase it.
  ...
  if not exists (select 1 from pg_constraint where conname='billing_handovers_confirmed_agrees') then
    alter table billing_handovers add constraint billing_handovers_confirmed_agrees
      check (case
               when state = 'invoiced'                     then invoice_confirmed_at is not null
               when state in ('to_prepare','handed_over')  then invoice_confirmed_at is null
               else true                                   -- cancelled: keep whatever happened
             end);
  end if;
...
begin
  ...
  -- invoice_confirmed_by and invoice_confirmed_at are DELIBERATELY LEFT ALONE.
  -- If Lone confirmed the invoice and this is cancelled afterwards, an invoice
  -- very likely exists in Sage and needs a credit note. Clearing the flag
  -- would destroy the only trace of it exactly when she needs to chase it.
  update billing_handovers
  ...
  return jsonb_build_object(
    'id', p_handover_id, 'state', 'cancelled', 'already_cancelled', false,
    -- The caller needs to know this, because it is the case that needs a
    -- human: the invoice is out there and somebody has to reverse it.
    'was_confirmed', h.invoice_confirmed_at is not null,
    'confirmed_at',  h.invoice_confirmed_at);
end $$;
