...
// Date.now(). Runs unchanged under Deno (Edge Function) and Node 22
// (tests/advance-recommendation.test.mjs).
//
// The primitives — the BURS table, the rate parser, liveLiabilities(),
// suggestClassification(), the formatters and the tier constants — moved to
// supabase/functions/_shared/kw-finance.ts on 3 Sep 2026 so the Debt Rehab
// Plan uses the same ones rather than a copy. Nothing was rewritten in the
// move, and every name this file used to export it still re-exports, so
// index.ts, report.ts, tests/ar-entry.ts and the 17-check unit suite import
// exactly as before.
//
...
// If advisor.html changes one of those, change it in kw-finance.ts in the
// same commit.
// ============================================================
import {
  ...
  calcAnnualTax, calcMonthlyPAYE, parseRate, liveLiabilities, suggestClassification,
  monthlyIncome, budgetCaptured,
} from "../_shared/kw-finance.ts";
import type {
  Assessment, Classification, RatePeriod, RawLiability, Tier,
} from "../_shared/kw-finance.ts";
...
export {
  DSR_GREEN_MAX, DSR_AMBER_MAX, DISPOSABLE_FLOOR_PCT, HIGH_COST_RATE_PA,
  DEFAULT_TERM_MONTHS, REPEATED_BORROWING_COUNT,
  pf, round2, fmtP, fmtPct,
  calcAnnualTax, calcMonthlyPAYE, parseRate, liveLiabilities, suggestClassification,
};
export type { Assessment, Classification, RatePeriod, RawLiability, Tier };

// ── The types this module owns ───────────────────────────────────
...
export function compute(a: Assessment, prep: Prep): Computed {
  ...
  // Income — identical to calcTotals(), via the shared summary
  const inc = monthlyIncome(income);
  const gross = inc.gross_salary, paye = inc.paye, other = inc.other_deductions, net = inc.net_salary;
  const spouse = inc.spouse_income, rentals = inc.rental_income,
        business = inc.business_income, dividends = inc.dividends;
  const totalIncome = inc.total_monthly_income;
  ...
  const captured = budgetCaptured(a.budget);
  const expenses = captured ? round2(budgetVals.reduce((s, v) => s + v, 0)) : null;
  const shortfall = captured && totalIncome > 0 ? (expenses as number) > totalIncome : null;
  if (!captured) gaps.add("Household budget not captured — a monthly shortfall cannot be ruled out");
  ...
  if (!captured) follow_up.push({ key: "budget", label: "Capture a household budget to confirm there is no monthly shortfall", on: true });
  ...
  return {
    ...
    budget: { captured, expenses, shortfall },
